define(["backbone","handlebars", 'picSure/settings', "text!header/header.hbs", "common/session", "picSure/userFunctions","picSure/applicationFunctions", "text!options/modal.hbs","text!header/userProfile.hbs", "util/notification"],
    ...
    var headerView = BB.View.extend({
        initialize: function () {
            ...
            $.ajax({
                url: '/picsureui/settings/settings.json',
                type: 'GET',
                contentType: 'application/json',
                success: function(res) { closure_this.picsureConfig = res; },
                error: function(res) {
                    handleAjaxError(res, failureMessage + (res.responseJSON.message?response.message:""), 6000);
                }
            });
        },
        events: {
        ...
        userProfile: function (event) {
            userFunctions.meWithToken(this, function(user){
                ...
                $("#token-drag-drop").on("dragstart", this.dragToken.bind(this));
            }.bind(this));
        },
        dragToken: function(event) {
            var settings = {};
            var psCfg = this.picsureConfig;
            var token = $('#user_token_textarea').attr("token");
            settings["Token"] = token ? token : JSON.parse(sessionStorage.session);
            settings["ResourceUUID"] = psCfg["picSureResourceId"] ? psCfg["picSureResourceId"] : "";
            try {
                settings["Endpoint"] = document.location.origin + psCfg.resources[0].basePath;
            } catch(e) {
                settings["Endpoint"] = document.location.origin + "/picsure";
            }
            settings = JSON.stringify(settings);
            event.originalEvent.dataTransfer.setData("application/json", settings);
